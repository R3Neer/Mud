...
[[notes/decisions/ADR-021-cycle-logical-lifespan-and-suspension-by-department|D-021]] retains complete properties when a structural dependency is destroyed. [[notes/decisions/ADR-077-cardinality-conditioned-destruction-and-transition-diagnostics|D-077]] resolves contained identities: removal is committed only if all final cardinalities and domains are valid; an immutable relationship retains latent membership, while a `mut` relationship removes it permanently. `create` restores only the former and validates the transition again.
