...
Una `action`, una rule reactiva o un `message` puede declarar locales puras mediante `:=` entre los metadatos y sus cláusulas principales. Son inmutables, secuenciales, visibles para locales posteriores y cláusulas posteriores y respetan las reglas ordinarias contra references adelantadas, ciclos y sombreado.
