...
This decision has been updated to the current vocabulary and grammar: it uses the short numeric types from [[ADR-067-short-names-for-numeric-types|D-067]], incorporates the intrinsic `name` from [[ADR-068-universal-thing-and-intrinsic-name|D-068]] and represents double-quoted literals in accordance with [[ADR-069-char-literals-with-double-quotes|D-069]]. In particular, the surface AST does not invent a distinct lexical node for `Char`; that elaboration requires type context.
