...
Mud starts from the idea that domain logic should work in much the same way.
...
operations, update the model atomically, validate it, rebuild derived artefacts
...
- **Reconstruction** — derived artefacts can be rebuilt from their source.
...
These are design goals under active formalisation, not claims about an already
