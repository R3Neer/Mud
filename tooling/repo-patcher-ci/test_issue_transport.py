...
def documents(package: bytes | None = None, chunk_chars: int = 24):
...
def reconstruct(issue, comments, **kwargs):
    options = {
...
class TransportTests(unittest.TestCase):
    def test_valid_single_chunk(self):
        package, _, issue, comments = documents(chunk_chars=100_000)
        rebuilt, _, report = reconstruct(issue, comments)
        self.assertEqual(rebuilt, package)
        self.assertEqual(report["actor"], ACTOR)
        self.assertNotIn("trigger_comment_id", report)

    def test_valid_multiple_chunks(self):
        package, _, issue, comments = documents(chunk_chars=20)
        rebuilt, _, report = reconstruct(issue, comments)
        self.assertEqual(rebuilt, package)
        self.assertGreater(report["chunk_count"], 1)

    def test_no_trigger_comment_is_required(self):
        package, _, issue, comments = documents(chunk_chars=100_000)
    ...
    def test_unrelated_comments_are_ignored(self):
        package, _, issue, comments = documents(chunk_chars=100_000)
        comments.extend([
            {"id": 90, "body": "hello", "user": {"login": "attacker"}},
            {"id": 91, "body": "/repo-patcher validate old", "user": {"login": ACTOR}},
        ])
        rebuilt, _, _ = reconstruct(issue, comments)
        self.assertEqual(rebuilt, package)

    def test_missing_chunk(self):
        _, _, issue, comments = documents(chunk_chars=20)
        comments.pop()
        with self.assertRaisesRegex(transport.TransportError, "Expected"):
            reconstruct(issue, comments)

    def test_duplicate_chunk(self):
        _, _, issue, comments = documents(chunk_chars=100_000)
        comments.append(deepcopy(comments[0]))
        ...
        issue["body"] = block(transport.REQUEST_START, transport.REQUEST_END, request)
    ...
    def test_invalid_base64(self):
        ...
        comments[0]["body"] = block(transport.CHUNK_START, transport.CHUNK_END, chunk)
    ...
    def test_unauthorized_issue_actor(self):
        _, _, issue, comments = documents(chunk_chars=100_000)
    ...
    def test_pull_request_is_rejected(self):
        ...
        with self.assertRaisesRegex(transport.TransportError, "Pull requests"):
            reconstruct(issue, comments)

    def test_wrong_actor_chunk_is_ignored(self):
        _, _, issue, comments = documents(chunk_chars=100_000)
    ...
    def test_trust_plugin_is_boolean(self):
        _, _, issue, comments = documents(chunk_chars=100_000)
        request = transport._extract_json_block(issue["body"], transport.REQUEST_START, transport.REQUEST_END, "request")
    ...
    def test_encode_round_trip_without_trigger_file(self):
        package = make_zip()
        with tempfile.TemporaryDirectory() as temp:
            ...
            args = transport.build_parser().parse_args([
                "encode", "--package", str(package_path), "--repository", REPOSITORY,
                "--target-sha", TARGET, "--request-id", REQUEST_ID,
                "--output-directory", str(output), "--chunk-chars", "20", "--trust-plugin",
            ])
            self.assertEqual(args.func(args), 0)
            self.assertFalse((output / "trigger-comment.md").exists())
            issue = {"number": 1, "state": "open", "body": (output / "issue-body.md").read_text(), "user": {"login": ACTOR}}
            comments = [
                {"id": i, "body": path.read_text(), "user": {"login": ACTOR}}
                for i, path in enumerate(sorted(output.glob("chunk-*.md")), 1)
            ]
            rebuilt, request, _ = reconstruct(issue, comments)
            self.assertEqual(rebuilt, package)
