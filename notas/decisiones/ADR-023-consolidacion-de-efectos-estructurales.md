...
## Varias creaciones de la misma regla

Las declaraciones de reglas no se fusionan dentro de una oleada. Dos solicitudes efectivas de creación de la misma identidad de regla constituyen un conflicto, incluso si sus cuerpos parecen iguales.

Esta condición no tiene que ser demostrable siempre en compilación:

- El compilador rechaza los conflictos que pueda probar.
- Puede aceptar programas cuya exclusión mutua no consiga demostrar.
- El runtime comprueba las solicitudes efectivas de cada oleada.
- Si aparecen dos, la transacción completa termina en `failed` y revierte.

La regla se aplica a reglas booleanas, reactivas y `always`.

La semántica de varias creaciones concurrentes del mismo alias permanece abierta.
...
- Un conflicto dinámico no produce commit ni estado parcial.

## Cuestiones abiertas

- Varias creaciones concurrentes del mismo alias.
...
7. Dos creaciones de la misma regla detectables estáticamente.
8. Dos creaciones de la misma regla detectadas solo en runtime.
