...
La forma contextual por identificador es válida y el tooling puede sugerir una abreviatura inequívoca más breve. Una sobrescritura de `name` idéntica al predeterminado recibe sugerencia de eliminación. Los miembros de family reciben el mismo `name: Text` intrínseco, sin alterar su identidad.

### Prefijos

`prefixes` omitido y `prefixes = empty` admiten ninguno. `prefixes = all` admite el catálogo completo de prefijos decimales SI vigente en MUD 1.0; una lista selecciona un subconjunto. Se incluyen desde `quecto` hasta `quetta`. Micro acepta `µ`, `μ` y `u` como entrada y normaliza a `µ`. No existen prefijos binarios ni composición de prefijos.
...
Las unidades prefijadas se elaboran estructuralmente y no reciben anclas adicionales.
