...
- C3: `especificacion/gramatica/mud.ebnf` retains `[ derived-value-shape ]`; `cobertura-sintactica.yaml` and `cst-a-ast-superficial.md` project that form; `mud-surface-ast.asdl` retains `derived_value_shape? shape` in `CalculatedFamilyDataDecl`.
