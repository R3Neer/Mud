...
| D-017 | Vigente | Todo tipo bien formado posee un valor predeterminado perteneciente a su dominio | [ADR-017](decisiones/ADR-017-valor-predeterminado-de-todo-tipo.md) | Decisión del autor, 2026-07-27; predeterminado de `Char` fijado el 2026-07-29 |
...
| D-019 | Vigente | La mutabilidad exterior de un lugar y la capacidad sobre los miembros de su colección son ortogonales; los roles `for` conservan ambos ejes | [ADR-019](decisiones/ADR-019-mutabilidad-ortogonal-de-coleccion-y-miembros.md) | Decisión del autor, 2026-07-27; extendida a roles `for` el 2026-07-29 |
...
| D-029 | Vigente | `*` designa el límite efectivo lateral; `[n]` conserva su lectura contextual de intervalo unitario; solo los puntos admiten `[a..b cycle)` | [ADR-029](decisiones/ADR-029-intervalos-estrellas-y-ciclos.md) | Decisión del autor, 2026-07-28; ambigüedad contextual de `[n]` precisada el 2026-07-29 |
...
| D-031 | Vigente | Todo alias es un tipo nominal inmutable; los estructurales componen su predeterminado a partir de los componentes | [ADR-031](decisiones/ADR-031-aliases-nominales-e-inmutables.md) | Decisión del autor, 2026-07-28; predeterminados de componentes añadidos el 2026-07-28 |
| D-032 | Vigente | `to` admite casting nominal; la construcción estructural parcial exige componentes nombrados y no coerciona valores ya tipados | [ADR-032](decisiones/ADR-032-construccion-contextual-y-casting-nominal.md) | Decisión del autor, 2026-07-28; construcción parcial nombrada precisada el 2026-07-28 |
...
| D-036 | Vigente | `on` vincula `thing` individuales; `for` admite cualquier tipo y vincula sus roles por identidad, valor o lugar | [ADR-036](decisiones/ADR-036-participantes-receptores-y-llamadas.md) | Migración normativa de las secciones 6 y 7; roles colectivos y de valor decididos por el autor el 2026-07-29 |
| D-037 | Vigente | Los campos almacenados ordenan tipo, dominio y colección; `:=` declara un cálculo puro con tipo opcional solo bajo inferencia unívoca y sugiere almacenamiento cuando sea equivalente | [ADR-037](decisiones/ADR-037-campos-y-dominios-declarativos.md) | Migración normativa de las secciones 13, 15 y 20; inferencia y sugerencia de almacenamiento decididas por el autor el 2026-07-29 |
| D-038 | Vigente | `family` declara tipos nominales finitos con datos inmutables almacenados o calculados; una `ordered family` puede aportar esos datos como claves de colección | [ADR-038](decisiones/ADR-038-familias-cerradas-de-valores.md) | Migración normativa de la sección 12; esquema, cálculos por miembro y claves de orden completados por decisión del autor el 2026-07-28 |
| D-039 | Vigente | Las colecciones forman multiconjuntos con álgebra conjuntista, inferencia de forma y propagación de `unique`, `ordered` y capacidad interior `mut` | [ADR-039](decisiones/ADR-039-colecciones-y-diccionarios.md) | Migración normativa de las secciones 16 a 18; álgebra de colecciones completada por decisión del autor el 2026-07-29 |
...
| D-041 | Vigente | `when` detecta `false → true`; `changes` pulsa por cambio neto sin estado propio y las vinculaciones inicializan su memoria según su origen | [ADR-041](decisiones/ADR-041-contratos-de-las-tres-clases-de-regla.md) | Migración normativa de las secciones 31 a 35; memoria inicial y pulsos precisados por el autor el 2026-07-29 |
...
| D-045 | Vigente en su núcleo | Las consecuencias se resuelven por ondas; una vinculación dinámica toma línea base en su primera onda activa antes de poder disparar | [ADR-045](decisiones/ADR-045-resolucion-causal-vinculaciones-y-cola.md) | Migración normativa de las secciones 45 a 49; nacimiento de memoria precisado el 2026-07-29 |
...
| D-049 | Vigente | La lógica usa `not`, `and`, `or`, `xor`, `=>` y `<=>`; intervalos y colecciones usan `|`, `&`, `^` y `-`, sin aliases verbales | [ADR-049](decisiones/ADR-049-operadores-precedencia-e-intervalos-normalizados.md) | Migración normativa de las secciones 27 a 29; escrituras canónicas únicas decididas por el autor el 2026-07-29 |
