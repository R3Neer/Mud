...
Cada `then` conserva su secuencialidad mediante un delta privado sobre la instantánea común. Los bloques no observan deltas parciales ajenos. Una vinculación local `nombre [: tipo] := expresión` lee la proyección privada alcanzada en su posición, se evalúa una vez y no produce un efecto ni se recalcula después. Solo puede depender de nombres ya visibles. Al consolidar los bloques, las activaciones mediante `create` preceden a las adiciones y las retiradas preceden a las destrucciones. Varias activaciones de una misma definición canónica ausente se consolidan idempotentemente; `create` no aporta cuerpos ni fragmentos declarativos. Los aliases no participan en efectos de ciclo de vida. Véanse [[notas/decisiones/ADR-023-consolidacion-de-efectos-estructurales|D-023]], [[notas/decisiones/ADR-054-definiciones-canonicas-y-activacion-inicial|D-054]], [[notas/decisiones/ADR-031-aliases-nominales-e-inmutables|D-031]] y [[notas/decisiones/ADR-066-valores-estaticos-y-vinculaciones-locales-en-then|D-066]].
