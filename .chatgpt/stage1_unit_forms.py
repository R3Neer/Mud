...
t=rd(rel)
old='''Identificador, nombre, plural, abreviatura y formas prefijadas deben ser inequívocos dentro de una magnitud. Una colisión entre magnitudes se resuelve mediante el tipo esperado o cualificación como `Length.meter`; sin contexto suficiente es un error.\n\nLa forma contextual por identificador es válida y el tooling puede sugerir una abreviatura inequívoca más breve. Una sobrescritura de `~name` idéntica al predeterminado recibe sugerencia de eliminación. Los miembros de `family` usan igualmente el metadato estándar `~name` sin alterar su identidad.\n'''
new='''Identificador, `~name`, `~plural` y `~abbreviation` participan bajo el mismo contrato cuando se habilitan como formas fuente. Una forma configurable puede contener espacios U+0020 y puntuación, pero debe contener al menos un carácter alfabético; por tanto no puede estar compuesta íntegramente por cifras ni íntegramente por caracteres no alfabéticos. Una forma completa que coincida exactamente con una palabra reservada dura no es admisible como forma fuente, aunque el mismo valor pueda seguir usándose como texto de presentación.\n\nLa unicidad dentro de una magnitud se comprueba sobre identificador, nombre, plural, abreviatura y todas las formas obtenidas después de aplicar cada prefijo permitido. Dos unidades distintas de la misma magnitud no pueden generar la misma forma, ni directamente ni por prefijado. Una colisión entre magnitudes distintas se resuelve mediante el tipo esperado o cualificación como `Length.meter`; sin contexto suficiente es un error.\n\nLa forma contextual por identificador es válida y el tooling puede sugerir una abreviatura inequívoca más breve. Una sobrescritura de `~name` idéntica al predeterminado recibe sugerencia de eliminación. Los miembros de `family` usan igualmente el metadato estándar `~name` sin alterar su identidad.\n'''
t=one(t,old,new,'D076 source forms')
wr(rel,t)
