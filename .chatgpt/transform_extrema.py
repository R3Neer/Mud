...
'''
p=ROOT/'notas/decisiones/ADR-095-extremos-vacios-como-ausencia-ordinaria.md'
if p.exists(): raise SystemExit('D-095 already exists')
...
t=t.replace(marker,marker+'- Modificada por: [[ADR-095-extremos-vacios-como-ausencia-ordinaria|D-095]]\n',1)
...
if 'D-095' not in t:
    t=t.replace(marker,marker+'- Modificada por: [[ADR-095-extremos-vacios-como-ausencia-ordinaria|D-095]] en la forma de resultado de `min` y `max` sobre ausencia.\n',1)
...
if needle in t and 'La semántica de ausencia de `min` y `max`' not in t:
    t=t.replace(needle,needle+' La semántica de ausencia de `min` y `max` es la de D-095: ningún candidato produce `empty` con cardinalidad `[0..1]`.',1)
w(p,t)

p='especificacion/08-sintaxis-abstracta.md'; t=r(p)
if '  - D-095\n' not in t:
    # add after latest known decision in provenance; D-094 exists on current main
    if '  - D-094\n' in t: t=t.replace('  - D-094\n','  - D-094\n  - D-095\n',1)
    else: t=t.replace('decisions:\n','decisions:\n  - D-095\n',1)
marker='## Magnitudes\n'
...
print('EXTREMA_EMPTY_TRANSFORM_OK')
